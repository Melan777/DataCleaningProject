...
```
git clone https://github.com/Melan777/DataCleaningProject
```
...
* Мельниченко Андрей (@Melan777 (Телеграм))

## Выводы

В итоге достигнуты следующие результаты по предварительной подготовке (очистке) наборов данных:
* Выявлены и обработаны пропущенные значения (путем удаления или замены на наиболее подходящие).
* Выявлены и обработаны выбросы (ошибки ввода).
* Удалены признаки и записи, которые не несут полезной информации 
В результате наборы данных подготовлены для анализа.
